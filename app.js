...
(function () {
  ...
  // ==========================================================================
  var ORTOFOTO_MAX_BYTES = 40 * 1000 * 1000; // 40 MB - TIFF/GeoTIFF costumam ser maiores que JPG/PNG
  var ORTOFOTO_MAX_MEGAPIXELS = 60; // protecao contra travar a aba com um raster gigante (TIFF comprimido pode ter poucos MB e ainda assim decodificar para uma imagem enorme)
  // Formatos aceitos na SELECAO do arquivo. Nem todos podem ser enviados "crus"
  // para a Anthropic/Vercel Blob (que só aceitam jpeg/png/webp/gif) - TIFF e
  // BMP são sempre decodificados no navegador e reconvertidos para PNG antes
  // do upload (ver ortofotoObterArquivoParaUpload). JPEG/PNG/WEBP/GIF seguem
  // sendo enviados como o arquivo original, sem reconversão.
  var ORTOFOTO_TIPOS_ENVIO_DIRETO = ["image/jpeg", "image/jpg", "image/png", "image/webp", "image/gif"];
  var ORTOFOTO_EXTENSOES_TIFF = [".tif", ".tiff"];
  var ORTOFOTO_EXTENSOES_ACEITAS = [".jpg", ".jpeg", ".png", ".webp", ".gif", ".bmp", ".tif", ".tiff"];
  var ORTOFOTO_CORES = ["#1d4ed8", "#0d9488", "#b7791f", "#c0362c", "#7c3aed", "#0891b2", "#65a30d", "#db2777"];
  var ORTOFOTO_SVGNS = "http://www.w3.org/2000/svg";

  state.ortofoto = {
    arquivo: null, // File original (mantido so pra exibir nome/tamanho ao usuario)
    arquivoParaUpload: null, // File/Blob realmente enviado ao analisar (original OU convertido para PNG)
    imagemUrl: null,
    imagemDataUrl: null,
    larguraNatural: 0,
    alturaNatural: 0,
  ...
  function ortofotoExtensaoDe(nomeArquivo) {
    var m = /\.[^.]+$/.exec(String(nomeArquivo || ""));
    return m ? m[0].toLowerCase() : "";
  }

  function ortofotoEhTiff(file) {
    var ext = ortofotoExtensaoDe(file.name);
    var mime = (file.type || "").toLowerCase();
    return ORTOFOTO_EXTENSOES_TIFF.indexOf(ext) !== -1 || mime === "image/tiff" || mime === "image/tif";
  }

  /** Converte um canvas em Blob (Promise) - callback do canvas.toBlob embrulhado. */
  function ortofotoCanvasParaBlob(canvas, tipo, qualidade) {
    return new Promise(function (resolve, reject) {
      canvas.toBlob(function (blob) {
        if (blob) resolve(blob); else reject(new Error("Falha ao gerar imagem a partir do canvas."));
      }, tipo, qualidade);
    });
  }

  /** Empacota um Blob (ex: vindo de canvas.toBlob) como File com nome - uploadToBlob()/@vercel/blob precisam de um "name" para o pathname do arquivo. */
  function ortofotoBlobComoArquivo(blob, nomeBase) {
    var nome = String(nomeBase || "ortofoto").replace(/\.[^.]+$/, "") + ".png";
    try {
      return new File([blob], nome, { type: blob.type || "image/png" });
    } catch (e) {
      // navegadores muito antigos sem suporte a "new File(...)" - anexa o nome manualmente no proprio Blob
      blob.name = nome;
      return blob;
    }
  }
  ...
  function handleOrtofotoArquivoSelecionado(file) {
    ...
    var ext = ortofotoExtensaoDe(file.name);
    if (ORTOFOTO_EXTENSOES_ACEITAS.indexOf(ext) === -1) {
      return ortofotoShowUploadError("Formato não suportado. Envie JPG, PNG, WEBP, GIF, BMP ou TIFF/GeoTIFF.");
    }
    if (file.size > ORTOFOTO_MAX_BYTES) {
      return ortofotoShowUploadError("Arquivo maior que 40 MB.");
    }
    ...
    if (ortofotoEhTiff(file)) {
      ortofotoCarregarTiff(file);
    } else {
      ortofotoCarregarRasterNativo(file);
    }
  }

  /** JPG/PNG/WEBP/GIF/BMP - formatos que o proprio navegador decodifica via <img>. */
  function ortofotoCarregarRasterNativo(file) {
    var o = state.ortofoto;
    o.imagemUrl = URL.createObjectURL(file);

    var probe = new Image();
    probe.onload = async function () {
      o.larguraNatural = probe.naturalWidth;
      o.alturaNatural = probe.naturalHeight;

      if (probe.naturalWidth * probe.naturalHeight > ORTOFOTO_MAX_MEGAPIXELS * 1e6) {
        ortofotoShowUploadError("Imagem muito grande (" + Math.round((probe.naturalWidth * probe.naturalHeight) / 1e6) + " megapixels). Reduza a resolução antes de enviar (limite deste protótipo: " + ORTOFOTO_MAX_MEGAPIXELS + " MP).");
        resetOrtofotoEstado();
        renderOrtofotoViewer();
        return;
      }
      ...
      // exportacoes (SVG/PNG) e, quando o formato original nao e aceito pela
      // Anthropic/Vercel Blob (ex: BMP), tambem usada para gerar o PNG que
      // efetivamente sera enviado na analise.
      var tipoOriginal = (file.type || "").toLowerCase();
      var enviarOriginal = ORTOFOTO_TIPOS_ENVIO_DIRETO.indexOf(tipoOriginal) !== -1;
      try {
        var canvas = document.createElement("canvas");
        canvas.width = probe.naturalWidth;
        canvas.height = probe.naturalHeight;
        canvas.getContext("2d").drawImage(probe, 0, 0);
        o.imagemDataUrl = canvas.toDataURL(tipoOriginal === "image/png" ? "image/png" : "image/jpeg", 0.92);

        if (enviarOriginal) {
          o.arquivoParaUpload = file;
        } else {
          // ex: BMP - a Anthropic/Vercel Blob nao aceitam, entao convertemos.
          var blobPng = await ortofotoCanvasParaBlob(canvas, "image/png");
          o.arquivoParaUpload = ortofotoBlobComoArquivo(blobPng, file.name);
        }
      } catch (e) {
        o.imagemDataUrl = null; // exportacoes de SVG/PNG ficam indisponiveis, mas a edicao continua normal
        o.arquivoParaUpload = enviarOriginal ? file : null;
      }
  ...
  /**
   * TIFF/GeoTIFF - o navegador nao decodifica nativamente via <img>, entao
   * usamos geotiff.js (global window.GeoTIFF, carregado via CDN no
   * index.html) para ler os pixels e "achatar" em um canvas comum. A partir
   * dai, o TIFF passa a ser tratado exatamente como qualquer outra imagem
   * (preview, edicao, exportacao, upload para analise em PNG).
   *
   * Limitacoes conhecidas deste protótipo: só as 3 (ou 4) primeiras bandas
   * são usadas como RGB(A) - imagens multiespectrais com mais bandas ou TIFF
   * com paleta de cores (indexado) não são interpretadas corretamente.
   */
  async function ortofotoCarregarTiff(file) {
    var o = state.ortofoto;
    if (!window.GeoTIFF || typeof window.GeoTIFF.fromArrayBuffer !== "function") {
      ortofotoShowUploadError("Biblioteca de leitura de TIFF não carregou. Verifique sua conexão e tente novamente.");
      resetOrtofotoEstado();
      renderOrtofotoViewer();
      return;
    }

    o.statusMsg = "Lendo TIFF...";
    document.getElementById("ortofoto-status").textContent = o.statusMsg;

    try {
      var buffer = await file.arrayBuffer();
      var tiff = await window.GeoTIFF.fromArrayBuffer(buffer);
      var image = await tiff.getImage();
      var width = image.getWidth();
      var height = image.getHeight();

      if (width * height > ORTOFOTO_MAX_MEGAPIXELS * 1e6) {
        throw new Error(
          "Imagem muito grande (" + Math.round((width * height) / 1e6) + " megapixels). " +
          "Reduza a resolução antes de enviar (limite deste protótipo: " + ORTOFOTO_MAX_MEGAPIXELS + " MP)."
        );
      }

      var samplesPerPixel = (typeof image.getSamplesPerPixel === "function" ? image.getSamplesPerPixel() : null) || 1;
      var rasters = await image.readRasters({ interleave: true });
      var rgba = ortofotoRasterParaRGBA(rasters, width, height, samplesPerPixel);
      ...
      o.larguraNatural = width;
      o.alturaNatural = height;
      o.imagemDataUrl = canvas.toDataURL("image/png");
      o.imagemUrl = o.imagemDataUrl; // <img src> aceita data URL normalmente
      var blobTiffPng = await ortofotoCanvasParaBlob(canvas, "image/png");
      o.arquivoParaUpload = ortofotoBlobComoArquivo(blobTiffPng, file.name);
      o.statusMsg = "";

      renderOrtofotoViewer();
    } catch (err) {
      ortofotoShowUploadError((err && err.message) || "Não foi possível ler este TIFF. Ele pode estar corrompido ou usar uma compressão não suportada.");
      resetOrtofotoEstado();
      renderOrtofotoViewer();
    }
  }
  ...
  /**
   * Converte o array de amostras lido por readRasters({interleave:true}) em
   * RGBA de 8 bits (Uint8ClampedArray), pronto para ImageData. Nunca calcula
   * geometria/coordenadas - so "achata" pixels para exibicao, exatamente como
   * o canvas faz para os demais formatos.
   */
  function ortofotoRasterParaRGBA(rasters, width, height, samplesPerPixel) {
    var n = width * height;
    var out = new Uint8ClampedArray(n * 4);
    var jaEm8Bits = (rasters instanceof Uint8Array) || (rasters instanceof Uint8ClampedArray);

    // Normalizacao simples (min/max -> 0..255) quando os valores nao sao
    // 8-bit "prontos" (ex: TIFF de 16 bits ou float). Para orto RGB comuns
    // (8-bit), os valores ja vem 0..255 e este passo e pulado.
    var lerAmostra;
    if (jaEm8Bits) {
      lerAmostra = function (v) { return v; };
    } else {
      ...
      var range = (max - min) || 1;
      lerAmostra = function (v) { return ((v - min) / range) * 255; };
    }

    for (var p = 0; p < n; p++) {
      var base = p * samplesPerPixel;
      var r, g, b, a = 255;
      if (samplesPerPixel === 1) {
        r = g = b = lerAmostra(rasters[base]);
      } else if (samplesPerPixel === 2) {
        // raro (grayscale + alpha) - trata a 1a amostra como intensidade
        r = g = b = lerAmostra(rasters[base]);
        a = lerAmostra(rasters[base + 1]);
      } else if (samplesPerPixel >= 4) {
        r = lerAmostra(rasters[base]);
        g = lerAmostra(rasters[base + 1]);
        b = lerAmostra(rasters[base + 2]);
        a = lerAmostra(rasters[base + 3]);
      } else {
        // samplesPerPixel === 3 (RGB) ou qualquer outro caso - usa as 3 primeiras bandas
        r = lerAmostra(rasters[base]);
        g = lerAmostra(rasters[base + 1]);
        b = lerAmostra(rasters[base + 2]);
      }
  ...
  async function ortofotoDetectarDivisas() {
    ...
    if (!o.arquivoParaUpload) {
      o.statusMsg = "Não foi possível preparar esta imagem para envio. Tente selecioná-la novamente.";
      document.getElementById("ortofoto-status").textContent = o.statusMsg;
      return;
    }
    ...
    try {
      var arquivoEnvio = o.arquivoParaUpload;
      var blobUrl = await withTimeout(
        uploadToBlob(arquivoEnvio, function (progress) {
      ...
      var resp = await withTimeout(
        fetch("/api/analisar-ortofoto", {
          ...
          body: JSON.stringify({
            filename: arquivoEnvio.name || o.arquivo.name,
            mimeType: arquivoEnvio.type || guessMimeFromName(arquivoEnvio.name || o.arquivo.name),
            blobUrl: blobUrl
          })
        }),
        150000,
